...
public class SAslashdimension_ex extends SpecialAttackBase implements IJustSpecialAttack,ISuperSpecialAttack{
    ...
    public void doSpacialAttack(ItemStack stack, EntityPlayer player) {
        ...
        if(entityId != 0){
            ...
            if(tmp != null){
                if(tmp.getDistanceToEntity(player) < 60.0f)
                    target = tmp;
            }
        }
        ...
        if(target == null) {
            ...
            player.addPotionEffect(new PotionEffect(Potion.moveSpeed.getId(),150,1,true));
            if(!player.worldObj.isRemote) {
                int level = EnchantmentHelper.getEnchantmentLevel(Enchantment.power.effectId, stack);
                float magicDamage = 2.0f + ItemSlashBlade.AttackAmplifier.get(tag) * (0.5f + level / 5.0f);
            ...
            bb = bb.expand(4.0f, 0.25f, 4.0f);//2,0.25,2
            ...
            if(!target.worldObj.isRemote){
                int level = EnchantmentHelper.getEnchantmentLevel(Enchantment.power.effectId, stack);
                float magicDamage = 2f + ItemSlashBlade.AttackAmplifier.get(tag) * (level / 5.0f);
                EntitySlashDimension dim = new EntitySlashDimension(world, player, magicDamage);
                if(dim != null){
    ...
    public void doJustSpacialAttack(ItemStack stack, EntityPlayer player) {
        ...
        if(target == null) {
            ...
            if(!player.worldObj.isRemote) {
                ...
                magicDamage = 2.0f + ItemSlashBlade.AttackAmplifier.get(tag) * (level / 5.0f);
                for(int i = 0; i < 5;i++){
            ...
            int level = EnchantmentHelper.getEnchantmentLevel(Enchantment.power.effectId, stack);
            float magicDamage = 2.0f + ItemSlashBlade.AttackAmplifier.get(tag) * (level / 5.0f);
            if(0 < level){
                for(int i = 0; i < 5;i++){
